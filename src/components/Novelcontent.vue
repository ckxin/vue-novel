<template>
  <div id="Novelcontent" class="bg-light">
    <!-- <div id="main-content" class="w-100" style="height:100%; background: #c7edcc"> -->
    ...
    <div id="catalogue" v-if="is_catalogue" @click="close_catalogue">
      <div id="catalogue-top" class="fixed-top bg-light p-3" style="width: 330px; font-size: 18px">
        目录
      </div>
      <div class="m-auto" v-if="catalogue_loading">正在生成目录</div>
      <div id="catalogue-titles" @click="go_chapter" v-else>
        <li class="bg-white p-3 border-bottom" v-for="(chapter_title, index) in chapter_titles" :key="index" :id="get_title_id(index)">
            {{ chapter_title.data }}
        </li>
      </div>
    </div>
    ...
    <div id="bookmark" v-if="is_bookmark" @click="close_bookmark">
      ...
      <div id="bookmark-list" v-else>
        <div class="bg-white p-3 border-bottom" v-for="(bookmark, index) in bookmark_list" :key="index" @touchstart="bookmark_process(bookmark, $event)" @touchend="cleartime(bookmark)">
    ...
    <div id="menu" v-if="is_menu" @click="close_menu">
      <div id="menu-top" class="row fixed-top bg-dark p-3">
        <div class="col-8 text-left">
          <img src="../../static/img/icon/fanhui_white.png" alt="返回" width="25px" @click="back_home">
        </div>
        <div class="col-2 text-center">
          <img src="../../static/img/icon/sousuo_white.png" alt="搜索" width="25px" @click="go_search">
        </div>
        <div class="col-2 text-center">
          <img :src="bookmark_icon" alt="书签" width="25px" @click="control_bookmark">
        </div>
      </div>
      <div id="menu-bottom" class="row fixed-bottom bg-dark py-3 px-4">
        ...
        <div class="col-3 px-0 text-center">
          <button class="btn btn-dark px-2" @click="go_prev">上一章</button>
        </div>
        <div class="col-3 px-0 text-center">
          <button class="btn btn-dark px-3" @click="open_catalogue">目录</button>
        </div>
        <div class="col-3 px-0 text-center">
          <button class="btn btn-dark px-3" @click="open_bookmark">书签</button>
        </div>
        <div class="col-3 px-0 text-center">
          <button class="btn btn-dark px-2" @click="go_next">下一章</button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    select_bg(event) {
      ...
      switch(bg) {
        case "日间": document.getElementById("main-content").className="bg-light"; break;
        case "护眼": document.getElementById("main-content").className="bg-green"; break;
        case "杏黄": document.getElementById("main-content").className="bg-yellow"; break;
        case "胭红": document.getElementById("main-content").className="bg-red"; break;
        case "海蓝": document.getElementById("main-content").className="bg-blue"; break;
        case "夜间": document.getElementById("main-content").className="bg-night"; break;
      }
    },
    // 减小小说字体
    down_fontsize() {
    ...
    async bookmark_process(bookmark, event) {
      ...
      this.Loop = setTimeout(() => {
        ...
        this.$nextTick(() => {
          document.getElementById("delete-bookmark").style.left = click_x + 20 + 'px';
  ...
  async mounted() {
    ...
    if(novel_style !== null) {
      ...
      let maincontent = document.getElementById("main-content");
      novelcontent.className = bgcolor;
      maincontent.style.fontSize = fontsize + "px";
      maincontent.className = bgcolor;
    }
    // 若有历史记录需修改
    let novel_info = JSON.parse(localStorage.getItem(this.novel_id));
    if(novel_info === null) {
      let res = await axios.post(CONF.host + "/novel_content", 
        { 
          novel_id: this.novel_id, 
      ...
      let novel_history = novel_info.novel_history;
      this.bookmark_list = novel_info.novel_bookmark;
  ...
  beforeRouteLeave (to, from, next) {
    ...
    if(this.chapter_num >= 3) {
      ...
      if(this.is_readfinish) {
        ...
        read_schedule = current_chapter_index / this.chapter_num;
      }
    } else {
      ...
      if(this.is_readfinish) {
        ...
        read_schedule = scroll_position / scroll_height;
      }
    }
    novel_history = {
      current_chapter_index,
